          ...
          <v-row dense>
            ...
            <v-col cols="3">
              <v-card
                class="ma-1"
                rounded="xl"
                :style="{ background: (!informationBar.idle) ? 'linear-gradient(to bottom right, rgba(52, 199, 89, 1), rgba(36, 138, 61, 1))' : 'rgba(255, 255, 255, 0.15)' }"
              >
                <v-list-item three-line>
                  <v-list-item-avatar
                    tile
                ...
                <v-list-item three-line>
                  ...
                  >
                    <v-btn
                      ...
                      :disabled="changeModeLoading"
                      @pointerup.prevent="changeMode"
                      @click="changeMode"
                    >
                      <svg viewBox="0 0 24 24"><title>lock</title><path d="M12,17A2,2 0 0,0 14,15C14,13.89 13.1,13 12,13A2,2 0 0,0 10,15A2,2 0 0,0 12,17M18,8A2,2 0 0,1 20,10V20A2,2 0 0,1 18,22H6A2,2 0 0,1 4,20V10C4,8.89 4.9,8 6,8H7V6A5,5 0 0,1 12,1A5,5 0 0,1 17,6V8H18M12,3A3,3 0 0,0 9,6V8H15V6A3,3 0 0,0 12,3Z" /></svg>
                    </v-btn>
                  ...
                  <v-list-item-content>
                  <v-list-item-title class="device-status-title">
                    <span>設備狀態</span>
                    <v-chip
                      x-small
                      label
                      class="connection-chip"
                      :color="informationBar.online ? 'green darken-1' : 'red darken-2'"
                      text-color="white"
                    >
  ...
  methods: {
    async get() {
      ...
      .then(data => {
        ...
        this.informationBar.idle        = data.idle
    ...
    async changeMode() {
      if (this.changeModeLoading) return
      this.changeModeLoading = true
      const enable = !this.informationBar.idle
      const ok = await ipcRenderer.invoke("main/changeMode", enable)
      eventBus.$emit('alert', {
        level: ok ? 'success' : 'error',
        message: ok
          ? (enable ? '切換為啟用中' : '切換為閒置中')
          : '模式切換失敗，請再試一次'
      })
      await sleep(400)
      this.changeModeLoading = false
    }
  },
